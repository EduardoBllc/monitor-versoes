...
## Status de execução (atualizado 2026-07-03)

Executando via **superpowers:subagent-driven-development**, na branch `motor-etapa1` (criada a
...
- ✅ **Tasks 1-13 completas e revisadas** (scaffolding, todo o domínio puro, portas, os dois
  fakes, os 5 serviços de núcleo — `TargetResolver`, `PresenceOracle`, `LockStore`,
  `BaseResolver`, `PublicationGate`). HEAD da branch: `f11fe1e`.
- ⏸️ **Pausado antes da Task 14** (`engine.Verificar`, primeiro ponto de integração de tudo) —
  o implementer foi interrompido pelo usuário antes de qualquer commit; nada a reverter, retomar
  normalmente com `scripts/task-brief` + dispatch do implementer.
- ⬜ Tasks 15-21 (demais operações do engine, `GitSubprocess`, `ManualList`, `ClickUpRest`, CLI)
  ainda não iniciadas.

### Correções encontradas durante a implementação (não estavam no plano original)

A revisão por task pegou dois bugs que vieram do **texto deste plano** (código dado verbatim nos
briefs), não de desvio dos implementers — registrados aqui pra quem retomar não repetir:
...
1. **Task 7 (`FakeGit.MergeBase`)** — o algoritmo original só achava merge-base quando uma branch
   ...
   deste projeto) retornavam erro. Corrigido pra uma LCA de verdade (conjunto de ancestrais de
   `a` + busca no caminho de `b`); teste de regressão `TestFakeGitMergeBaseBranchesDivergentes`
   adicionado. Também corrigido nessa mesma leva: `ListVersionBranches` tinha ordem não-determinística
   (iteração de map) — agora ordenado.
2. **Task 9 (`TargetResolver`)** — o fixture de teste original não provava nada sobre
   ...
   rodar, então o teste passava pelo motivo errado. Corrigido com um fixture que gera colisão de
   substring de verdade no `SearchCommits` (só o word-boundary do `MatchExato` resolve), e uma
   checagem extra de "o teste tem dentes" (desligar `MatchExato` temporariamente e confirmar que
   falha).

Se este plano for reaproveitado em outro projeto, os blocos de código das Tasks 7 e 9 acima
**já estão com essas correções aplicadas** nas seções abaixo — não precisa reintroduzir o bug.

### Nota aberta pra Task 17

Task 11 (`LockStore.Reconstruir`) tem uma lacuna de cobertura conhecida, não corrigida (Minor,
não bloqueou aprovação): o fixture só testa 1 commit por chamado, então o acúmulo de múltiplos
commits no mesmo chamado durante a reconstrução nunca foi exercitado — a ordem pode sair
reverso-cronológica (já que `CommitsInRange` caminha do tip pra base) sem que nada reordene
depois. Ao chegar na Task 17 (`ReconstruirLock`), considerar um fixture de integração com
chamado multi-commit antes de aprovar.
