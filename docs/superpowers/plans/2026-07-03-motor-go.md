...
## Status de execução (atualizado 2026-07-13 — CONCLUÍDO)

Executado via **superpowers:subagent-driven-development**, na branch `motor-etapa1` (criada a
partir de `master` após a Task 1). Progresso completo e granular (SHAs, achados de review) fica
em `.superpowers/sdd/progress.md` — este bloco é só o resumo.

- ✅ **Todas as 21 tasks completas e revisadas** — motor (domínio, portas, serviços, engine) +
  os 3 adapters reais (`GitSubprocess`, `ManualList`, `ClickUpRest`) + CLI (`cmd/motor`).
- ✅ **Revisão final de branch (whole-branch, 28 commits vs `master`)** rodada depois das 21
  tasks — achou 3 problemas cross-cutting que nenhuma revisão por task veria (cada uma só olha
  o diff de uma task isolada). Todos corrigidos e re-revisados limpos.
- ✅ **Suíte inteira verificada pelo controller de forma independente** (não só confiando no
  relato dos subagents): `go build ./...` limpo, `go test ./...` — 52 testes, 7 pacotes, tudo
  verde, `gofmt`/`go vet` limpos.
- Próximo passo: `superpowers:finishing-a-development-branch` (decidir merge/PR/cleanup).

### Correções encontradas durante a implementação (não estavam no plano original)

Sete bugs reais surgiram durante a execução — a maioria veio do **texto deste plano** (código
dado verbatim nos briefs), não de desvio dos implementers. Registrados aqui pra quem reaproveitar
este plano não reintroduzir nenhum:

1. **Task 7 (`FakeGit.MergeBase`)** — o algoritmo original só achava merge-base quando uma branch
   era ancestral direta da outra; branches genuinamente divergentes (o caso normal de versões
   deste projeto) retornavam erro. Corrigido pra uma LCA de verdade; teste de regressão
   `TestFakeGitMergeBaseBranchesDivergentes` adicionado. Junto: `ListVersionBranches` tinha ordem
   não-determinística (iteração de map) — agora ordenado.
2. **Task 9 (`TargetResolver`)** — o fixture de teste original não provava nada sobre
   `MatchExato`: o `SearchCommits` já filtrava o commit "errado" sozinho antes do `MatchExato`
   rodar. Corrigido com um fixture que gera colisão de substring de verdade no `SearchCommits`
   (só o word-boundary do `MatchExato` resolve).
3. **Task 11/17 (`LockStore.Reconstruir`)** — acúmulo de múltiplos commits no mesmo chamado saía
   em ordem reverso-cronológica (`CommitsInRange` caminha do tip pra base, nada reordenava depois).
   Corrigido reusando `domain.OrdenarPorData`; teste de regressão com 2 commits no mesmo chamado.
4. **Task 18 (`GitSubprocess.CherryPickX`)** — **Critical**: invertia a classificação de
   conflito-vs-rerere-auto-resolvido. Git nunca chama `--continue` por conta própria mesmo quando
   `rerere.autoUpdate` já resolveu e re-stageou tudo — o código tratava "sem arquivos em conflito
   após falha" como erro real, tornando o caminho de auto-continue do §8 inatingível na prática.
   Corrigido usando `PendingCherryPick()` (checa `CHERRY_PICK_HEAD`) em vez de `ConflictedPaths()`
   pra classificar; teste de regressão com replay genuíno de rerere em dois worktrees. Junto:
   `PredictMerge` tratava qualquer código de saída não-zero do `merge-tree` como conflito de
   conteúdo (cego a erros de ferramenta reais); `PatchID` vazava o subprocess irmão em caminhos
   de erro precoce do pipe.
5. **Pós-Task-21 (`GitSubprocess.WriteFile`)** — sempre rodava `git commit` incondicional; falhava
   com "nothing to commit" sempre que o conteúdo do lock não mudava — acontece em toda `criar` de
   versão "Fechada" sem nenhum cherry-pick necessário (caso comum). Corrigido checando
   `git diff --cached --quiet` antes de comitar.
6. **Revisão final (`Incrementar`/`IncrementarContinue`) — Critical**: num lote com múltiplos
   commits, se um conflito no meio bloqueasse, os commits já aplicados antes dele nunca eram
   gravados no lock (lock em lote só escreve no fim de um lote bem-sucedido) — e
   `IncrementarContinue` só registrava o commit que acabou de resolver, perdendo os anteriores
   pra sempre (mesmo estando corretos no git). **Não mudou a decisão de "lock em lote"** — o fix
   foi só no `IncrementarContinue`, que agora chama `LockStore.Reconstruir` (já existente) pra
   recompor o lock inteiro a partir do git em vez de registrar um commit só.
7. **Revisão final (`PresenceOracle`/`Verificar`) — Important**: erro de `CommitsInRange` (ex.:
   objeto sumiu do histórico após rebase/gc) abortava `Verificar` inteiro em vez de reportar via
   `CommitsSumidos`/`LockIntegro`. Corrigido pra tratar como "ausente", matching o próprio
   fallback do design (§2 "senão → ausente"). Junto (Minor): `Conflitantes` podia vazar commits
   sumidos só-no-lock, violando o invariante documentado de que é subconjunto de `Faltantes` —
   restringido ao lado alvo.

Se este plano for reaproveitado em outro projeto, os blocos de código abaixo **já estão com essas
7 correções aplicadas** — não precisa reintroduzir nenhuma.

### Gaps aceitos, não corrigidos (documentados, não bugs)

- Adapter `SavedView` (§4) não implementado — `ManualList`/`ClickUpRest` cobrem as duas pontas do
  fallback; `SavedView` fica pra quando/se os outros dois se mostrarem insuficientes.
- Linha "pick manual sem o tool" da tabela de reconciliação (§9) não implementada — exigiria
  escanear todos os trailers da branch a cada `verificar`, não só checar presença dos commits
  esperados.
- `ClickUpRest.CampoChamadoID` precisa do ID real do campo "Numero do chamado" confirmado numa
  conta ClickUp de verdade antes de uso em produção — hoje é configurável, não hardcoded.

### Recomendações pra Etapa 2 (daemon), não bloqueantes

- `GitSubprocess.currentBranch` é estado mutável compartilhado (setado por `WorktreeAdd`/
  `UseWorktree`, lido por `CherryPickX` e afins) — seguro pra uso sequencial via CLI, mas duas
  operações concorrentes na mesma instância correriam risco de race no daemon. Ou tornar a branch
  parâmetro explícito desses métodos, ou uma instância de `GitSubprocess` por operação (é só um
  `RepoPath` string, barato).
- Paridade comportamental `FakeGit`↔`GitSubprocess` nunca foi verificada além da linha do
  adapter — aceitável hoje porque o invariante do projeto (branches de versão são só
  cherry-pick, histórico linear) faz `FakeGit`'s modelagem simplificada (grafo linear, match por
  substring) ser fiel na prática. Um smoke test de `engine` contra `GitSubprocess` de verdade
  seria seguro barato antes do daemon.
