# TUI para verificar e atualizar versões

**Status:** design aprovado e direção visual A2 aprovada em conversa em 2026-08-20
...
`verificar` ou `atualizar`, vendo seus resultados de forma estruturada. A TUI é uma fachada
...
- execução de `verificar` e `atualizar` com Tickio;
...
A TUI chama `motor.engine.verificar.verificar()` e `motor.engine.atualizar.atualizar()`,
renderizando os valores retornados. O CLI continua responsável apenas por argparse e
impressão. O motor e seus tipos não ganham conceitos de widget, cor ou interação.
...
Falha no fetch deixa o seletor de versão vazio, mantém as ações desabilitadas e
...
│ MOTOR                                            ? ajuda   q sair  │
├────────────────────────────────────────────────────────────────────┤
│ repo [──────────▾] / versão [────────▾] [Verificar] [Atualizar]    │
├────────────────────────────────────────────────────────────────────┤
│ banner e contadores                                                │
...
- `Verificar` e `Atualizar` ficam sempre visíveis; não existe menu genérico de ações.
- `Verificar` permanece desabilitado até repo e versão válidos estarem selecionados.
- `Atualizar` só é habilitado quando a última verificação encontrou commits faltantes
  e a versão não está liberada. Seu rótulo mostra a quantidade aplicável.
...
- Durante uma carga, verificação ou atualização, os controles ficam bloqueados. A
  primeira operação precisa terminar antes de outra começar.
- A navegação padrão por teclado do Textual é preservada; `v` verifica, `u` atualiza e
  `q` encerra a aplicação.

## Execução de `verificar` e `atualizar`

Ao pressionar uma ação, o worker:
...
6. chama `verificar(deps, versao, auditar=auditar_tag)` ou `atualizar(deps, versao)`;
7. fecha sessão e engine ao terminar, inclusive em erro.

Os dois fluxos compartilham apenas a composição de `Deps`; regras de verificação,
cherry-pick, push e conflito permanecem nos engines existentes.
...
- status amarelo `Pendências encontradas` nos demais casos, sem painel vermelho;
- resumo compacto de escopo, commits faltantes, conflitos e chamados sem commits;
- alerta de estado divergente, incluindo hashes de commits sumidos;
- alertas de tasks ambíguas e tasks sem commits;
- commits faltantes agrupados em uma tabela por chamado e preservando sua ordem;
...
- em auditoria: indicação explícita de que a tag foi recalculada sem alterar o
  snapshot;
- em atualização: commits aplicados agrupados por chamado ou orientação para usar
  `--continue` na CLI quando um conflito bloquear o lote.

O resumo permanece visível para leitura rápida. A interface não inventa um total de
chamados que o `VersionStatus` não forneça de forma autoritativa.
...
- Repo sem versões mostra `nenhuma branch ou tag X.Y.Z encontrada` e mantém as ações
  desabilitadas.
...
4. Testes com `App.run_test()` e bordas falsas percorrem seleção de repo, seleção
   de versão, verificação, habilitação da atualização e renderização dos resultados.
...
- Verificar chama o engine existente e apresenta claramente verde, pendências,
  conflitos, suspeitas e inconsistências.
- Atualizar só fica disponível após uma verificação aplicável, chama o engine
  existente e apresenta commits aplicados ou o bloqueio por conflito.
